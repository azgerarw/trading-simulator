...
class Layout:
    def __init__(self, root, symbol, df):
        ...
        self.max_quantity = Positions(state.current_user[0]).fetch_by_symbol(symbol=symbol)[0]
        print(self.max_quantity)

    def build(self):

        self.sell_window = Toplevel(self.root)
        self.sell_window.title(f"{self.symbol}: sell order")
        
        self.sell_window_frame = Frame(self.sell_window, padx=20, pady=20, width=300, height=400)
        self.sell_window_frame.grid(row=0, column=0, sticky="nsew")
        self.sell_window_frame.grid_propagate(False)
        for row in range(7):
            self.sell_window_frame.rowconfigure(row, weight=1)
        for col in range(2):
            self.sell_window_frame.columnconfigure(col, weight=1)

        self.symbol_label = Label(self.sell_window_frame, anchor="center", text=f"{self.symbol} / USD")
        self.symbol_label.grid(row=0, column=0, sticky="nsew", columnspan=2)

        self.last_price_label = Label(self.sell_window_frame, anchor="center", text=f"Last price: {self.df.iloc[-1]["Close"]:.2f}$")
        ...
        self.quantity_label = Label(self.sell_window_frame, text="Quantity", anchor="center")
        self.quantity_label.grid(row=3, column=0, columnspan=2, sticky="nsew")

        self.total_var = StringVar(value="Total: 0")

        self.total_label = Label(self.sell_window_frame, textvariable=self.total_var)
        ...
        self.sell_button = ttk.Button(self.sell_window_frame, bootstyle="primary", text="Sell")
        self.sell_button.grid(row=6, column=0, columnspan=2, sticky="nsew")